...
def _build_segments_chart(resumen_segmentos: list[dict]) -> Image | None:
    ...
    # Right: monthly benefit per segment
    ...
    img_buf = io.BytesIO()
    fig.savefig(img_buf, format="png", dpi=150, bbox_inches="tight", facecolor="#ffffff")
    plt.close(fig)
    img_buf.seek(0)

    usable_w = PAGE_W - 2 * MARGIN
    return Image(img_buf, width=usable_w, height=usable_w * 0.42)
